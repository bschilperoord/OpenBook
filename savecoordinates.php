...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    // Retrieve the x and y coordinates from the POST data
    ...
    // For example, you can store the coordinates in a database

    // Simulate a response
    $response = "Coordinates received and processed successfully.";

    // Return a response back to the JavaScript code
    echo $response;
} else {
...
$query = "SELECT COUNT(*) FROM settings WHERE name = 'sidebarcoordinates' AND userid = ?";
...
if ($stmt) {
    ...
    if ($count === 0) {
        ...
        $name = 'sidebarcoordinates';
        $value = "$x, $y"; // Change this to the desired default theme value
...
$userID = $_SESSION['userid'];
$sidebarcoordinates = "$x, $y";

// Prepare the update query
$query = "UPDATE settings SET value = ? WHERE name = 'sidebarcoordinates' AND userid = ?";
$stmt = $conn->prepare($query);

// Bind the values to the prepared statement
$stmt->bind_param("si", $sidebarcoordinates, $userID);
...
if (isset($_SESSION['userid'])) {
    ...
    $query = "SELECT value FROM settings WHERE name = 'sidebarcoordinates' AND userid = ?";
    $stmt = $conn->prepare($query);
